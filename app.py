import streamlit as st
import pandas as pd
import yfinance as yf
from sklearn.ensemble import GradientBoostingRegressor

# --- CONFIGURATION ---
TICKERS = ["RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "INFY.NS", "ICICIBANK.NS", 
           "SBIN.NS", "BHARTIARTL.NS", "LT.NS", "HCLTECH.NS", "KOTAKBANK.NS"]

st.set_page_config(page_title="Project Alpha: Core", layout="wide")
st.title("🏛️ Project Alpha: Institutional Committee")

@st.cache_data(ttl=3600)
def get_clean_data():
    master_data = []
    for ticker in TICKERS:
        try:
            # Download ticker data
            df = yf.download(ticker, period="6mo", progress=False)
            
            # Defensive check: ensure data is not empty and has enough rows
            if df.empty or len(df) < 20: continue
            
            # Flatten columns if MultiIndex (avoids ValueError)
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.get_level_values(0)
            
            # Calculate Indicators
            df['SMA_20'] = df['Close'].rolling(20).mean()
            df['RSI'] = 100 - (100 / (1 + (df['Close'].diff().clip(lower=0).rolling(14).mean() / 
                                          df['Close'].diff().clip(upper=0).abs().rolling(14).mean())))
            df['RVOL'] = df['Volume'] / df['Volume'].rolling(20).mean()
            
            df['Ticker'] = ticker
            master_data.append(df.tail(1))
        except: continue
        
    return pd.concat(master_data).dropna() if master_data else pd.DataFrame()

# --- AI COMMITTEE (MOD 10-11) ---
def generate_report(data):
    # Committee scoring engine
    data['AI_Score'] = (data['RSI'] < 45).astype(int) * 40 + (data['RVOL'] > 1.2).astype(int) * 30 + 30
    data['Weight'] = (data['AI_Score'] / data['AI_Score'].sum())
    return data[['Ticker', 'Close', 'AI_Score', 'Weight']]

# --- EXECUTION ---
data = get_clean_data()

if not data.empty:
    report = generate_report(data)
    st.subheader("📋 Daily Institutional Recommendations")
    st.dataframe(report.style.format({'Weight': '{:.1%}', 'Close': '₹{:.2f}'}), use_container_width=True)
else:
    st.error("Engine failure: Market data sync is currently unavailable. Please try again.")
