...
def get_clean_data():
    ...
    for ticker in TICKERS:
        try:
            ...
            # Validation: Ensure data is usable
            if df is None or df.empty or len(df) < 20: continue
            
            # Ensure columns are flattened to avoid MultiIndex ValueErrors
...
def generate_report(data):
    # Committee scoring engine: Combining indicators into a conviction score
...
if not data.empty:
    ...
    st.dataframe(report.style.format({'Weight': '{:.1%}', 'Close': '₹{:.2f}'}), use_container_width=True)
else:
    st.error("Engine failure: Market data sync is currently unavailable. Please refresh in 60 seconds.")
